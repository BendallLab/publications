...
def process_csv(file_path):
    ...
    df_filtered['TMA_Region'] = df_filtered['TMA'].astype(str) + '_' + df_filtered['Region'].astype(str)

    # Averaging the z-scores for each TMA+Region combination
    pivot_zscore = df_filtered.pivot_table(index='channel', columns='TMA_Region', values='z_score', aggfunc='mean', fill_value=0)

    # Plotting the heatmap for averaged z-scores
    plt.figure(figsize=(15, 10))
    sns.heatmap(pivot_zscore, cmap='viridis', center=0)
